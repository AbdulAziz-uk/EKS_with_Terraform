# EKS(Elastic Kubernetes Service) setup with Terraform. #
...
## Step 1: Create a VM on Cloud/local computer and access it, perform the following task. 
...
terraform -version
```
### 📦 Enter Security Credentials of AWS user to access AWS resources from AWS CLI
Enter Security key, Security Access key, Region.
```bash
aws configure
```
...
```
Change directory to EKS_with_Terraform
```bash
...
## 🌍 Run Terraform to create EKS cluster along with resources:

```bash
terraform init
terraform plan -f main.tf
terraform apply -f main.tf
```
It will create 19 resources on AWS.
